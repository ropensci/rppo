...
)
{
  ...
  for(i in 1:length(sp)){
    ...
    sample.size <- as.numeric(nrow(sub))
    data$sample.size[data$scientificName == sp[i] &
                     data$measurementType == trait] <- sample.size
    data[, "sample.size"] <- as.numeric(data[, "sample.size"])
    
    ##label if the sample size is not sufficient
    if(isTRUE(sample.size < n.limit |
    ...
    else if(isTRUE(sample.size > 5000)){ 
    ...
    ##now testing if less than 5K and more than then minimum sample size needed
      ...
      normal <- shapiro.test(sub$measurementValue[sub$measurementType == trait]) #normality test
      if(isTRUE(normal[[2]] < 0.05)){ #if significantly different, then not normal
        data$normality[data$scientificName == sp[i] &
                       data$measurementType == trait & 
                       !(data[, "measurementStatus"] %in% status)] <- "non-normal"
      }
      else if(isTRUE(normal[[2]] >= 0.05)){ #if not significantly different, then normal
        data$normality[data$scientificName == sp[i] &
                     data$measurementType == trait & 
                     !(data[, "measurementStatus"] %in% status)] <- "normal"
      }
  ...
  for(i in 1:length(sp)){
    ...
    data[, "meanValue"] <- as.numeric(data[, "meanValue"])
    ...
    data[, "sdValue"] <- as.numeric(data[, "sdValue"])
